//! `rabbit` — run a single Rabbit burrow from the command line.
//!
//! # Usage
//!
//! ```text
//! rabbit serve                     # serve from ./config.toml
//! rabbit serve --config path.toml  # serve from a specific config
//! rabbit serve --port 8443         # override the listening port
//! rabbit init                      # generate a starter config.toml
//! rabbit info                      # show burrow identity
//! ```
...
/// Rabbit burrow engine — run a text-based, peer-to-peer node.
#[derive(Parser)]
...
"#;
    ...
    println!("Run `rabbit serve` to start your burrow.");
    Ok(())
}
